...
public class UIPlayAnimation : MonoBehaviour
{
	...
	void OnEnable()
	{
		...
		if (toggle != null) toggle.onChange.AddListener(OnToggle);
	}
...
#endif
		UIToggle toggle = GetComponent<UIToggle>();
		if (toggle != null) toggle.onChange.RemoveListener(OnToggle);
	}
	...
	void OnToggle(UIToggle toggle, bool state)
	{
		if (!enabled) return;
		if (trigger == Trigger.OnActivate || (trigger == Trigger.OnActivateTrue && state) || (trigger == Trigger.OnActivateFalse && !state)) Play(state, dualState);
	}
