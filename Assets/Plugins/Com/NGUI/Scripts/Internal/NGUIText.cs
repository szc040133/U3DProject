...
public static class NGUIText
{
	...
	[System.Diagnostics.DebuggerHidden]
	[System.Diagnostics.DebuggerStepThrough]
	public static Color ParseColor(string text, int offset) { return ParseColor24(text, offset); }
	...
	public static void Print(string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (symbol != null)
			{
				...
				int offsetY = Mathf.Min(0, finalSize - symbol.height);
				if (offsetY == 0) offsetY -= 2;	//为等于28像素的字体特殊处理
				if (offsetY < -8) offsetY += 2; //为大于36像素的图片特殊处理
				v1y = -(y + (symbol.offsetY + offsetY) * fontScale);
